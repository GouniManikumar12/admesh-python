...
pip install git+ssh://git@github.com/GouniManikumar12/admesh-python.git
```

> [!NOTE]
> Once this package is [published to PyPI](https://app.stainless.com/docs/guides/publish), this will become: `pip install --pre admesh`
...
to add `ADMESH_API_KEY="My API Key"` to your `.env` file
...
You can enable logging by setting the environment variable `ADMESH_LOG` to `info`.

```shell
$ export ADMESH_LOG=info
```
...
recommend = response.parse()  # get the object that `recommend.get_recommendations()` would have returned
...
```python
import httpx
from admesh import Admesh, DefaultHttpxClient

client = Admesh(
    # Or use the `ADMESH_BASE_URL` env var
